...
class OptimizerComparison:
    ...
    def run_foga(self):
        ...
        cmd = ['python3', 'foga.py', self.source_file]
        ...
        try:
            ...
            # Parse output to extract best time
            output = result.stdout
            print(output)
            
            # Extract best execution time from output
            best_time = float('inf')
            evaluations = 0
    ...
    def run_hbrf(self):
        ...
        cmd = ['python3', 'hbrf_optimizer.py', self.source_file]
        if self.test_input_file:
            cmd.append(self.test_input_file)
        ...
        try:
            ...
            if os.path.exists('hbrf_results.json'):
                with open('hbrf_results.json', 'r') as f:
                    ...
                    best_time = hbrf_data.get('best_time', float('inf'))
                    evaluations = hbrf_data.get('total_evaluations', 0)
            else:
                # Parse from output
                best_time = float('inf')
                evaluations = 0
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("Example: python compare_optimizers.py matrix_multiply.c")
        print("         python compare_optimizers.py program.cpp input.txt")
        sys.exit(1)
